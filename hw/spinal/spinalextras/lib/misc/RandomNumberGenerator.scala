...
class RandomNumberGenerator(withSeed: Boolean = false) extends Component {
  val io = new Bundle {
    ...
    val valid = out Bool()
    val seed = withSeed generate new Bundle {
      val value = in UInt(64 bits)
  ...
  if(withSeed) {
    when(io.seed.enable) {
      lfsr := io.seed.value | 1  // Ensure non-zero
    }
    io.lfsrState := lfsr
  }
...
case class RandomNumberGeneratorApb3(baseAddress: Int = 0) extends Component {
  ...
  rng.io.seed.value := lfsrControlHigh @@ lfsrControlLow
  rng.io.seed.enable := busCtrl.isWriting(0x04) || busCtrl.isWriting(0x08)
