...
class RandomNumberGeneratorTest extends AnyFunSuite {
  test("RNG produces varying random numbers") {
    Config.sim.withVerilator.doSim(new RandomNumberGenerator) { dut =>
      ...
      for (i <- 0 until 100) {
        ...
        val value = dut.io.random.payload.toBigInt
        samples.add(value)
        sequence.append(value)
        assert(dut.io.random.valid.toBoolean, "Valid should always be true")
      }
  ...
  test("RNG seeding produces different sequences") {
    Config.sim.withVerilator.doSim(new RandomNumberGenerator) { dut =>
      ...
      for (_ <- 0 until 20) {
        dut.clockDomain.waitSampling()
        seq1.append(dut.io.random.payload.toBigInt)
      }
      ...
      for (_ <- 0 until 20) {
        dut.clockDomain.waitSampling()
        seq2.append(dut.io.random.payload.toBigInt)
      }
      ...
      for (_ <- 0 until 20) {
        dut.clockDomain.waitSampling()
        seq3.append(dut.io.random.payload.toBigInt)
      }
  ...
  test("RNG uniform distribution - comprehensive statistical test") {
    Config.sim.withVerilator.doSim(new RandomNumberGenerator) { dut =>
      ...
      for (_ <- 0 until numSamples) {
        dut.clockDomain.waitSampling()
        samples.append(dut.io.random.payload.toBigInt)
      }
