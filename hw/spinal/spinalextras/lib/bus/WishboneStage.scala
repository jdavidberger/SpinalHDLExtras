...
object WishboneStage {
  def apply(bus : Wishbone): Wishbone = {
      ...
      PipelinedMemoryBusToWishbone(adapter_pmb, 1)
  }
}

class WishboneStageTestDut extends Component {
  val cfg = WishboneConfig(32, 32, addressGranularity = AddressGranularity.WORD)
  val io = new Bundle {
    val mbus = master(Wishbone(cfg))
    val sbus = slave(Wishbone(cfg))
  }

  WishboneStage(io.mbus) << WishboneStage(io.sbus)
}

class WishboneStageTest extends AnyFunSuite {
  Config.sim.doSim(
    new WishboneStageTestDut
  ) { dut =>
    SimTimeout(5000 us)
